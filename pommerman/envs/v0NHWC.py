...
class Pomme(gym.Env):
    ...
    def _set_observation_space(self):
        ...
        - agent's position (equals to one of {Agent0.value, Agent3.value})
        - player ammo counts (tiled to board_size^2)
        - blast strength (tiled to board_size^2)
        - can_kick (tiled to board_size^2)
        - teammate (equals to one of {AgentDummy.value, Agent3.value}).
    ...
    def featurize(obs):
        ...
        blast_strength = utility.make_np_float([obs["blast_strength"]])
        blast_strength = np.broadcast_to(blast_strength, board.shape)
        can_kick = utility.make_np_float([obs["can_kick"]])
        can_kick = np.broadcast_to(can_kick, board.shape)
